...
import click
from app.crawlers import base, browsers, utils, proxy
...
@celery.task(name='crawlers.tjmg', default_retry_delay=5 * 60,
...
def tjmg_command(**kwargs):
  ...
  if enqueue:
    utils.enqueue_tasks(tjmg_task, split_tasks, **kwargs)
  else:
    tjmg_task(**kwargs)
