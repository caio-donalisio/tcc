...
if os.path.isfile('session_page_number'):
	...
	d = f.read()
	print(d)
	dateid = int(d.split(':')[0])
	page_num = int(d.split(':')[1])
	initial_range = dateid
	f.close()
...
# Range de datas do pendulum
start_date = pendulum.datetime(2020, 1, 1) # TODO: change date
end_date = pendulum.datetime(2020, 2, 1)

period = pendulum.period(start_date, end_date)

dates = period.range("days") # TODO: change from days to months

ranges = []

for dt in dates:
  ranges.append([dt.format('DD/MM/YYYY'), dt.add(days=1).format('DD/MM/YYYY')]) # TODO: change from days to months

ranges = ranges[initial_range:]

for idx, date_range in enumerate(ranges):
	print(f'using range: {"-".join(date_range)}')
	
	# set search based off of the date range
	driver.get('https://esaj.tjsp.jus.br/cjsg/consultaCompleta.do')
	wait.until(EC.presence_of_element_located,((By.ID,'iddados.buscaInteiroTeor')))
	search_box = driver.find_element_by_id('iddados.buscaInteiroTeor')
	search_box.send_keys('a ou de ou o')
	start_box = driver.find_element_by_id('iddados.dtPublicacaoInicio')
	start_box.send_keys(date_range[0])
	end_box = driver.find_element_by_id('iddados.dtPublicacaoFim')
	end_box.send_keys(date_range[1])
	wait.until(EC.presence_of_element_located,((By.ID,'pbSubmit')))
	search_button = driver.find_element_by_id('pbSubmit')
	search_button.click()
	logging.info('[+] OK - TJSP Crawler initialized')
	...
	while True:
		...
		if first_run:
			should_stop = False
			while True:
				ementaclass = driver.find_element_by_class_name('ementaClass')
				ementahtml = ementaclass.get_attribute('innerHTML')
				if 'encontrado nenhum' in ementahtml:
						print('no acordaos for this range, skipping')
						should_stop = True
						break
